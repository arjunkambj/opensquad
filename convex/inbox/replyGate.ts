 ...
 *   rules run: a workspace whose agent is paused must still stop mailing
 ...
 * - `before_connection` — older than `workspaces.connectedAt`, or the
 *   workspace has no connection time at all.
...
async function threadStartedByUs(
  ...
  return attempts.some(
    (attempt) =>
      ...
      attempt.workspaceId === conversation.workspaceId,
  );
}
 ...
 * workspace's connection time — except the sender, which is compared against
...
export async function evaluateReplyHistory(
  ...
  const { conversation, workspace, receipt } = args;
  ...
  const connectedAt = workspace.connectedAt;
  ...
  // the workspace's own inbox is the one address a reply may never come from.
  ...
  const ourAddress = parseInboundSender(workspace.inboxRef);
 ...
 * would put our words in front of someone the workspace never chose to
