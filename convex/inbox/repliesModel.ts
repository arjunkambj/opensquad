 ...
 * dedupes on `(workspaceId, requestId)`, so a re-driven step replays the
 ...
 *
 * `(workspaceId, inboxRef, providerMessageRef)` is the message's identity
...
export async function findInboundReceipt(
  ...
  const rows = await ctx.db
    ...
    .withIndex("by_workspace_inbox_providerMessageId", (q) =>
      q
