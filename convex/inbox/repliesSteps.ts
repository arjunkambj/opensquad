 ...
 * unsubscribe is honoured with the workspace at zero credits and the kill
...
export const handleInboundReply = internalMutation({
  ...
  handler: async (ctx, args): Promise<HandleInboundReplyResult> => {
    ...
    if (workspace === null) {
      return { outcome: "skipped:workspace_missing" };
    }
  ...
  handler: async (ctx, args): Promise<ApplyFreeRuleResult> => {
    ...
    if (args.kind === "unsubscribe") {
      ...
      await noteOnThread(
        ctx,
        conversation,
        `Opt-out honoured: ${targets.size === 1 ? "this address is" : "the address that asked and this thread's outbound contact address are both"} suppressed for the workspace, and the lead is closed. Remove the suppression in Settings → Blocklist to contact them again.`,
      );
      return { applied: true };
    }
    ...
    await noteOnThread(
      ...
      failed === undefined
        ...
        : `The mail system reported a permanent delivery failure (rule ${rule}). That address is suppressed for the workspace and the lead is closed.`,
    );
    return { applied: true };
  },
});
