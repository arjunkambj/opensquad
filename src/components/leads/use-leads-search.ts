...
import { exclusiveFilters, withLeadFilters } from "./leads-model"

const LEADS_ROUTE = "/_dashboard/_org/leads"
...
export function useLeadsSearch() {
  ...
  const goTo = (patch: Partial<LeadsSearch>) => {
    void navigate({
      to: "/leads",
      search: (current: LeadsSearch) => ({ ...current, ...patch }),
    })
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void navigate({
        to: "/leads",
        search: (current: LeadsSearch) =>
          withLeadFilters(current, {
  ...
  return {
    ...
    applyFilter: (patch: Partial<LeadsSearch>) => {
      setTrail([])
      ...
      void navigate({
        to: "/leads",
        search: (current: LeadsSearch) =>
          withLeadFilters(current, exclusiveFilters(applied)),
      })
    },
    ...
    setPageSize: (size: PageSize) => {
      ...
      void navigate({
        to: "/leads",
        search: (current: LeadsSearch) =>
          withLeadFilters(current, { limit: size }),
      })
    },

    /** Back to page one of the same filters, for a cursor that outlived its page. */
    resetPaging: () => {
      setTrail([])
