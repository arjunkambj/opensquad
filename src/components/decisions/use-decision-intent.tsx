...
/**
 * Client request ids, one per (decision, action) intent.
 *
 * The rule the backend enforces and this hook has to match: a requestId is a
 * *logical intent*, not a click. `approvals.approve/requestChanges/reject` and
 * `decisions.resolve` all replay a recorded outcome for a repeated requestId,
 * and `approvals` raises CONFLICT when one requestId is replayed carrying a
 * different verdict. So:
 *
 ...
 * - A different decision needs a different id, so the map is keyed by both.
 *
 ...
 * component is keyed on the decision id by its parent, so a new decision
 * mounts a new map.
 */
export function useDecisionIntents() {
  const intents = useRef(new Map<string, string>())

  return useCallback((decisionId: string, action: string): string => {
