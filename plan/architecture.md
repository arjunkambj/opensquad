...
| `conversations` | `workspaceId`, `inboxRef`, `employeeId`, `state: open|closed|unassigned`, `humanTakeover`, `contextVersion`, `unreadCount`, `createdAt`, `updatedAt`; `prospectId?`, `campaignId?`, `assigneeIdentityKey?`, `takeoverReason?`, `takeoverBy?`, `takeoverAt?`, `lastDisposition?`, `lastDispositionAt?`, `lastInboundFrom?`, `providerThreadRef?`, `currentDraftId?`, `lastInboundMessageRef?`, `lastInboundAt?`, `lastMessageAt?` | `by_workspaceId_and_state_and_lastMessageAt`, `by_workspaceId_and_humanTakeover_and_lastMessageAt`, `by_inboxRef_and_providerThreadRef`, `by_workspaceId_and_state_and_humanTakeover`, `by_prospectId`; unique assigned inbox/provider-thread mapping |
| `conversationNotes` | `workspaceId`, `conversationId`, `kind: note|system`, `actor`, `body`, `createdAt` | `by_conversationId_and_createdAt`; append-only internal notes and conversation lifecycle records; a note can never resolve a business approval |
...
| `emailEventReceipts` | `workspaceId`, `inboxRef`, `providerEventId`, `applicationKey`, `providerMessageRef`, `eventType`, `receivedAt`, `direction: inbound|outbound`, `handlingState: pending|handled|failed`, `providerFacts`; `providerThreadRef?`, `handledAt?`, `error?` | `by_providerEventId`, `by_workspaceId_and_applicationKey`, `by_providerMessageRef`, `by_direction_and_handlingState_and_receivedAt`; unique provider event and application handling key; bounded replay/reconciliation record; `direction` is derived from `applicationKey` so the inbound drain ranges over its own half rather than filtering a page after taking it |
| `quarantinedEmailEvents` | `inboxRef`, `providerEventId`, `applicationKey`, `providerMessageRef`, `eventType`, `reason: inbox_unassigned|inbox_ambiguous`, `receivedAt`, `state: quarantined|released|discarded`; `providerThreadRef?`, `providerTimestamp?`, `releasedAt?`, `releasedTo?`, `note?` | `by_providerEventId`, `by_inboxRef_and_state`, `by_state_and_receivedAt`; a verified event whose inbox no workspace claims (or which two claim) has no workspace for a receipt, and dropping it loses the mail — the provider will not resend an already-ingested `event_id`. Provider identifiers only; the message stays in the mail component, and `internal.quarantine.replayForInbox` replays through the ordinary receipt path once an assignment exists |
...
Conversation ownership is two separate facts: `employeeId` assigns the machine and `assigneeIdentityKey` assigns the human, and an assignee must resolve to an active workspace membership before it is stored. `associateProspect` freezes `campaignId` on the conversation, so re-campaigning a lead later cannot silently retarget reply work already in flight; the association validates that the lead belongs to that campaign and to this workspace. `takeoverReason`/`takeoverBy`/`takeoverAt` explain a frozen thread on the row itself, because `activityEvents`, `decisions` and `missionComments` all require a `missionId` and an unassigned conversation has no mission — its lifecycle is recorded in `conversationNotes` instead, which never surfaces in the decision queue. Attention counts read the two disjoint exact ranges of `by_workspaceId_and_state_and_humanTakeover` and are bounded (`50+`), never a computed total.

Do not create a duplicate custom `messages` table. Authorize inbound component queries through workspace conversation/inbox references; compose outgoing history from immutable drafts and send attempts. `emailEventReceipts` supplies application replay and early-event reconciliation because the app-owned sender does not create a component outbound row. Use `incoming:<inbox>:<message>` as the inbound application key so a second event ID for one message cannot start a second reply mission. Delivery receipts retain only necessary verified facts and never another copy of full message bodies. Keep durable approved content beyond component cleanup periods. A verified event whose inbox resolves to no workspace, or to more than one, is quarantined in `quarantinedEmailEvents` rather than dropped and replayed through the same receipt path once the assignment exists.
...
The `research` row's `{sourceUrl,retrievedAt,excerpt,confidence}` are synthesized by the backend from the OpenSquad-owned Firecrawl retrieval and are not accepted from the worker (P21; integrator decision D3). `vResearchResult.observations` supplies only `{topic,finding,sourceUrl?}`, so `convex/evidence.ts` `recordResearchEvidence` is the one synthesis site: an observation becomes an `evidence` row only when it cites a page **the backend retrieved and this run read back**, and the url, retrieval time, excerpt, status code and truncation flag are read back from that page's own `providerOperations` receipt rather than from the caller. "This run read it back" is mechanical: the receipt names the run, either because this run paid for the retrieval (`providerOperations.runId`) or because it replayed the receipt and was recorded on it (`providerOperations.replayedForRunIds`). The `operationKey` is campaign-scoped, so a campaign's second mission replays every page it plans and pays for none of them; strict run equality would have made those pages invisible to the run reading them and no campaign could be researched twice. An observation with no `sourceUrl`, or naming a page the backend never fetched, is not evidence — it is counted as `unsourced` and belongs in the research brief, labelled. `confidence` is never model-supplied: `supported` requires a 2xx, untruncated retrieval **this run itself paid for** (strict equality on `providerOperations.runId`), `hypothesis` requires the reserved `hypothesis:` topic marker the Researcher template instructs the model to use, and everything else is `unknown` — including every page a run replayed rather than retrieved, whose retrieval predates the run and which nothing re-checked. `qualification` and its reason live on `prospects.qualification`/`prospects.fitReason`; the concise brief is an `artifacts` row of kind `research_brief`. `vResearchResult` is deliberately not widened — widening it would put these columns back under model control.

The worker input envelope is composed by `convex/lib/roleTemplates.ts` (P21), and the composition is not the obvious one: `input.context[]` never reaches the model — `worker/src/daemon.ts` passes only `input.prompt` to `turn/start` and `parseClaimedWork` drops the context blocks — so every block is rendered INTO `prompt` server-side and `context[]` carries the structured mirror. `workerRequests.inputRef.value.prompt` is therefore the exact recorded text the model received. The host frame is the only text in it with instruction authority; workspace instructions, the campaign brief, the business profile, the prospect, retrieved pages and stored evidence are each a labelled `<<<BLOCK label=…>>>` body. Block labels come from a closed host set and every body has the fence markers stripped before emission, so a scraped page or an inbound signature cannot close its own block and open one that claims to be the frame. `input.capabilities` is refused at dispatch and mirrored from the set `dispatchWorkerRequest` derived from the run's own employee row, so no renderer can name its own capability set.
