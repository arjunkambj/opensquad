...
The `research` row's `{sourceUrl,retrievedAt,excerpt,confidence}` are synthesized by the backend from the OpenSquad-owned Firecrawl retrieval and are not accepted from the worker (P21; integrator decision D3). `vResearchResult.observations` supplies only `{topic,finding,sourceUrl?}`, so `convex/evidence.ts` `recordResearchEvidence` is the one synthesis site: an observation becomes an `evidence` row only when it cites a page **this run's backend actually retrieved**, and the url, retrieval time, excerpt, status code and truncation flag are read back from that page's own `providerOperations` receipt rather than from the caller. An observation with no `sourceUrl`, or naming a page the backend never fetched, is not evidence — it is counted as `unsourced` and belongs in the research brief, labelled. `confidence` is never model-supplied: `supported` requires a 2xx, untruncated retrieval in this run, `hypothesis` requires the reserved `hypothesis:` topic marker the Researcher template instructs the model to use, and everything else is `unknown`. `qualification` and its reason live on `prospects.qualification`/`prospects.fitReason`; the concise brief is an `artifacts` row of kind `research_brief`. `vResearchResult` is deliberately not widened — widening it would put these columns back under model control.
